    ...
    def analyzeSentenses(self):
        ...
        for key in self.structure["Знаки препинания"]:
            if (key in endOfSentensesMarks):
                ...
                for sentense in map(lambda sentense: (sentense.strip() + key, len(re.sub(r"[^A-zА-я, +] ", "", sentense.strip()).split(" "))) if (len(re.sub(r"[^A-zА-я+]", "", sentense).strip()) > 0) else (), self.text.split(key)):
                    if (len(sentense) > 0):
...
paragraphs = splitTextIntoParagraphs(data["Предложения"], countSentensesInParagraph)